...
-- JM CHANGES
hook.Add("TTTPlayerSpeedModifier", "JM_SpeedBoostFromWeapon", function(ply, _, _, speedMultiplierModifier)
	if not IsValid(ply)then return end
   	local wep = ply:GetActiveWeapon()
	if not IsValid(wep) then return end
	if wep.MoveMentMultiplier == nil then return end
	if not isnumber(wep.MoveMentMultiplier) then return end
	speedMultiplierModifier[1] = speedMultiplierModifier[1] * wep.MoveMentMultiplier
end)