-- Give these files out to clients (As they contain Client Code)
AddCSLuaFile()

JM_ShopCreditCosts = {
    ["weapon_jm_equip_awp"] = 3,
    ["weapon_jm_equip_beartrap"] = 3,
    ["weapon_jm_equip_landmine"] = 3,
    ["weapon_jm_equip_throwing_knives"] = 3,
    ["weapon_jm_equip_c4"] = 2,
    ["weapon_jm_equip_newtonlauncher"] = 2,
    ["weapon_jm_equip_orb_fire"] = 2,
    ["weapon_jm_equip_chameleon"] = 2,
    ["weapon_jm_equip_soap"] = 2,
    ["weapon_jm_equip_supercrowbar"] = 2,
    ["weapon_jm_equip_rollermine"] = 2,
    ["weapon_jm_equip_poisondart"] = 2,
    ["weapon_jm_equip_propbomb"] = 2,
    ["weapon_jm_equip_huntersense"] = 2,
    ["weapon_jm_equip_doom_dart"] = 2,

    ["weapon_jm_equip_taser"] = 3,
    ["weapon_jm_equipment_grenade_flash"] = 3,
    ["weapon_jm_zloot_traitor_tester"] = 3,
    ["weapon_jm_equip_tree"] = 2,
    ["weapon_jm_equip_agentmaker"] = 2,
    ["weapon_jm_equip_placer_carepackage"] = 2,
    ["weapon_jm_equip_carpenter"] = 2
}