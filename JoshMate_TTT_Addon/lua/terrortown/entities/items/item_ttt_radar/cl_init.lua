...
function RADAR:Draw(client)
	...
	if self.jmHudWarnings_count ~= 0 then
		for _, jmHudWarning in pairs(self.jmHudWarnings) do
			...
			if client:GetTeam() == TEAM_SPEC or not client:Alive() or jmHudWarning.whoCanSee == 0 then

				DrawTargetJmVersion(jmHudWarning, 24, 0, true)

			elseif jmHudWarning.whoCanSee == 1 and client:GetRole() == ROLE_TRAITOR then

				DrawTargetJmVersion(jmHudWarning, 24, 0, true)

			elseif jmHudWarning.whoCanSee == 2 and client:GetRole() == ROLE_DETECTIVE then
