...
if CLIENT then
	...
	function DISGUISE.CreateMenu(parent)
		...
		local owned = LocalPlayer():HasEquipmentItem("item_jm_passive_disguiser")
	...
	hook.Add("TTTEquipmentTabs", "TTTItemDisguiser", function(dsheet)
		...
		if not LocalPlayer():HasEquipmentItem("item_jm_passive_disguiser") then return end
...
if SERVER then
	local function SetDisguise(ply, cmd, args)
		...
		if not ply:HasEquipmentItem("item_jm_passive_disguiser") then return end
