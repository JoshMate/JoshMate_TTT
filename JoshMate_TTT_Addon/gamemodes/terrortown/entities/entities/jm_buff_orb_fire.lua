...
local JM_PrintName              = JM_Global_Buff_Orb_Fire_Name
local JM_BuffNWBool             = JM_Global_Buff_Orb_Fire_NWBool
local JM_BuffDuration           = JM_Global_Buff_Orb_Fire_Duration
local JM_BuffIconName           = JM_Global_Buff_Orb_Fire_IconName
local JM_BuffIconPath           = JM_Global_Buff_Orb_Fire_IconPath
local JM_BuffIconGoodBad        = JM_Global_Buff_Orb_Fire_IconGoodBad
...
hook.Add("TTTPlayerSpeedModifier", ("JM_BuffSpeedEffects_".. tostring(JM_PrintName)), function(ply, _, _, speedMultiplierModifier)
    if ply:GetNWBool(JM_BuffNWBool) == true then 
	    speedMultiplierModifier[1] = speedMultiplierModifier[1] * 0.4
    end 
end)
