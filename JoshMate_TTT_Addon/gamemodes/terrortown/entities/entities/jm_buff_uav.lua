...
local JM_PrintName              = JM_Global_Buff_Uav_Name
local JM_BuffNWBool             = JM_Global_Buff_Uav_NWBool
local JM_BuffDuration           = JM_Global_Buff_Uav_Duration
local JM_BuffIconName           = JM_Global_Buff_Uav_IconName
local JM_BuffIconPath           = JM_Global_Buff_Uav_IconPath
local JM_BuffIconGoodBad        = JM_Global_Buff_Uav_IconGoodBad
...
-- ESP Halo effect
hook.Add( "PreDrawHalos", "Halos_UAV", function()

    local players = {}
     local count = 0
 
    for _, ply in ipairs( player.GetAll() ) do
        if (ply:IsTerror() and ply:Alive() and LocalPlayer():GetNWBool(JM_BuffNWBool) ) then
             count = count + 1
             players[ count ] = ply
        end
    end
