...
function SWEP:Initialize()
	if CLIENT then
		...
		if isfunction(self.AddTTT2HUDHelp) then
			self:AddTTT2HUDHelp("Scan for a sample", "Change Active Sample")
			self:AddHUDHelpLine("Delete a sample", Key("+reload", "R"))
		else
			ErrorNoHalt("[TTT2][ERROR] You are using an add-on that overwrites the 'weapon_jm_base_gun.lua' file while not providing essential functions. That will lead to several incompatibilites.")
		end
	end
...
function SWEP:AddPlayerSample(corpse, killer)
	...
	if self.karmaRewardClaimed == false then
		...
		JM_Function_Karma_Reward(self:GetOwner(), JM_KARMA_REWARD_ACTION_DNASCAN, "DNA scan")
	end
...
if SERVER then
	...
	local function ScannerFeedback()
		if not LocalPlayer():HasWeapon("weapon_jm_equip_dna") then return end

		local scanner = LocalPlayer():GetWeapon("weapon_jm_equip_dna")
	...
	local function ScannerUpdate()
		...
		if not client:HasWeapon("weapon_jm_equip_dna") then return end
