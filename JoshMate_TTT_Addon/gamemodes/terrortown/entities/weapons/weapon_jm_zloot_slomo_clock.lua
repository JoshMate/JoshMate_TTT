...
SWEP.PrintName				= "Time Control Clock"
SWEP.Author			    	= "Josh Mate"
SWEP.Instructions			= "Change time for 15 sconds"
SWEP.Spawnable 				= true
SWEP.AdminOnly 				= true
...
function SWEP:SpeedUpTime()
	if SERVER then
		game.SetTimeScale(2.00)
		-- Timer To set time back to normal (Or it happens at the start of next round)
		if timer.Exists("Timer_SloMo_Clock") then timer.Destroy("Timer_SloMo_Clock") end
		timer.Create( "Timer_SloMo_Clock", (JM_SlowMo_Clock_Duration*2), 1, function() game.SetTimeScale(1) end )
		sound.Play("effect_speedup_start.mp3", self:GetOwner():GetPos(), 150, 100)
		self:Remove()
	end
end
...
if CLIENT then
	function SWEP:Initialize()
	   self:AddTTT2HUDHelp("Slow down time", "Speed up time", true)
