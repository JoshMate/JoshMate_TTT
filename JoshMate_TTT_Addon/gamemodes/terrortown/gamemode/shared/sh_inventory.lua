...
function InventorySlotFree(ply, kind)
	...
	-- Josh Mate Change: Nade Mule Hack
	if invSlot == WEAPON_NADE and JM_CheckIfPlayerHasBuff("jm_buff_karmabuff_extra_grenade", ply) then
		slotCount = (GetGlobalInt(ORDERED_SLOT_TABLE[invSlot]) + 1)
	else
		slotCount = GetGlobalInt(ORDERED_SLOT_TABLE[invSlot])
	end
	-- End of hack
